...
这个 skill 让 Codex 通过当前 skill 目录下的 `scripts/qiaoya` runtime 查询敲鸭社区的公开课程、AI 日报和用户态能力。优先使用结构化 JSON 输出，再基于结果做总结。
...
在执行命令时，优先使用当前 skill bundle 内的脚本：
...
~/.codex/skills/qiaoya/scripts/qiaoya public course-get <course-id> --json
```
...
~/.codex/skills/qiaoya/scripts/qiaoya ai-news history --json
~/.codex/skills/qiaoya/scripts/qiaoya ai-news daily --date 2026-04-20 --json
```
...
~/.codex/skills/qiaoya/scripts/qiaoya notification unread --json
```
...
- 如果 `scripts/qiaoya` 不存在，提示需要先运行 `npx qiaoya`
