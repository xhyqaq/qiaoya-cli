...
如果识别到资源是视频，不要假设自己已经理解了视频内容。优先判断是否已有现成文字：
- 如果页面、接口或资源里已经有字幕、文稿、摘要，优先使用现成文字
- 如果只有视频，没有可用文字材料，不要假装总结视频内容
...
当命中视频场景且缺少文字材料时，默认这样提示用户：

> 检测到这是视频资源，我更适合处理文字内容。  
> 如果你愿意，我可以先把视频转成文字，再继续帮你总结和分析。  
> 默认会使用本地 `faster-whisper` 转写能力。

更短的轻提示版本：

> 这是视频资源。若你愿意，我可以先转成文字，再继续帮你分析。

注意：
- 不要默认替用户安装转写模型，先征得同意
- 不要把“我可以转写”说成已经完成转写
- 一旦用户同意，后续链路应是“视频/音频 -> 转写文本 -> AI 总结”
...
- 对视频资源，不要假装已经看懂视频；先判断是否有字幕或文稿，没有就提示可用本地 `faster-whisper` 转写
